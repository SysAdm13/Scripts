#Скрипт создает RDP ярлык, нужен ввод имени пользователя и номер компьютера.

$user = Read-Host "Введите имя пользователя без указания домена. Например: ***"   #указать пример формата имени пользователя
$remoteComputer = Read-Host "Введите номер комьютера без указания домена. Например: ***"   #указать пример формата имени компьютера
$gateway = "" #указать шлюз
$domain = ""  #указать домен
